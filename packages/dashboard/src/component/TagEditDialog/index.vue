...
const open = () => {
  visible.value = true;
  tags.value = (tagStore.tagList ?? []).map(t => ({ ...t, apps: t.apps.map(a => ({ ...a })) }));
  appOptions.value = buildAppOptions();
  activeIndex.value = tags.value.length ? 0 : -1;
};

const addTag = () => {
  tags.value.push({ name: "", apps: [], enabled: false, color: PREDEFINE_COLORS[0] });
...
const handleSubmit = async () => {
  ...
  try {
    await tagStore.setTags(
      tags.value.map(t => ({ name: t.name.trim(), apps: t.apps.map(a => ({ ...a })), enabled: t.enabled, color: t.color }))
    );
